"""
Copyright 2020 Elias Fernández Domingos
...
Created by: Elias F. Fernandez
Created on: 13/03/2019

This python module estimates the expected payoff of
some given strategies in the Collective-Risk Dilemma,
when playing against each other in a population of 2 strategies.
"""

import numpy as np

actions = np.asarray([0, 2, 4])


# Let's estimate the payoffs through simulation

def always_zero(prev_contribution_group, threshold, t):
    return actions[0]


def always_two(prev_contribution_group, threshold, t):
    return actions[1]


def always_four(prev_contribution_group, threshold, t):
    return actions[2]


def compensator(prev_contribution_group, threshold, t):
    """
    If prev_donations > threshold -> contribute 0
    else contribute 4
    """
    if t == 0:
        return actions[1]
...
def reciprocal(prev_contribution_group, threshold, t):
    """
